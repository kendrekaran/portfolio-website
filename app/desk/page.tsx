...
export const metadata: Metadata = {
  ...
  openGraph: { title: "My desk · Karan Kendre", description: "The gadgets, tools, and little things I own.", url: "/desk" },
  twitter: { card: "summary_large_image", title: "My desk · Karan Kendre", description: "The gadgets, tools, and little things I own.", creator: "@karankendre", site: "@karankendre", images: [{ url: "/desk/opengraph-image", alt: "Karan Kendre’s desk – keyboard, mouse, and chair" }] },
}
  ...
  return <div className="quiet-page quiet-wide">
    ...
    <main id="main-content">
      ...
      <div className="desk-grid">{deskItems.map((item, index) => <article className="desk-item" key={item.url}>
        <a className="desk-photo" href={item.url} target="_blank" rel="noopener noreferrer" tabIndex={-1} aria-hidden="true"><span className="desk-number">{String(index + 1).padStart(2, "0")}</span><img src={item.image} alt="" width="400" height="320" loading={index < 2 ? "eager" : "lazy"} /><span className="desk-photo-arrow" aria-hidden="true"><ArrowUpRightIcon /></span></a>
        <span className="quiet-eyebrow">{item.category}</span>
        <h2><a href={item.url} target="_blank" rel="noopener noreferrer">{item.name} <span aria-hidden="true"><ArrowUpRightIcon /></span></a></h2>
        <p>{item.detail}</p>
        <a className="quiet-small quiet-muted" href={item.url} target="_blank" rel="noopener noreferrer" aria-label={`View ${item.name} on Amazon`}>Find it on Amazon <ArrowUpRightIcon /></a>
      </article>)}</div>
